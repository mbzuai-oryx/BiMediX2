...
INSTRUCT_PROMPT = """We would like to request your feedback on the performance of two AI assistants in response to the user question displayed above in 'Moders Standard Arabic' language. The user asks the question on observing an image. For your reference, the visual content in the image is represented with Arabic caption describing the same image.
  Please rate the helpfulness, relevance, accuracy, level of details of their responses. Each assistant receives an overall score on a scale of 1 to 10, where a higher score indicates better overall performance. Deduct score if the answer is in a language other than 'Moders Standard Arabic'.
...
def infer(samples):
    ...
    # model_inst = llm.GPT("gpt-4o")
    model_inst = llm.GPT("gpt-4o-2024-08-06")
...
if __name__ == '__main__':
   parser = argparse.ArgumentParser("GPT-4o Multimodal Chat Scoring", add_help=True)
   parser.add_argument("--answers-file", default="", metavar="FILE", help="path to model answer file")
   parser.add_argument("--question-file", default="../data/test_sets/bimed-mbench_ara.jsonl", metavar="FILE", help="path to multichat questions file")
   parser.add_argument("--scores-file", default="", metavar="FILE", help="path to save gpt-4o score file")
