...
def main(args):
    ...
    out_file = args.scores_file.replace('_score.jsonl','_results.txt')
    ...
    for q_type, score_dict in score_type_dict.items():
        result[q_type]['gpt4o_score'] = util.get_avg(score_dict[1])
        result[q_type]['pred_score'] = util.get_avg(score_dict[2])
        result[q_type]['pred_relative_score'] = util.get_avg([float(s2)/(float(s1)+1e-100) for s1, s2 in zip(score_dict[1], score_dict[2])])*100
    ...
    with open(out_file, 'w') as f:
      f.write(table_str)
    print(f"\nResults saved to \033[1m{out_file}\n")

if __name__ == '__main__':
   parser = argparse.ArgumentParser("GPT-4o Multimodal Chat Eval Postprocessing", add_help=True)
   parser.add_argument("--scores-file", default="", metavar="FILE", help="input path to gpt-4o score file")
